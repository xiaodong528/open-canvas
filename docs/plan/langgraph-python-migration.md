...
> **创建日期**: 2025-12-16
> **完成日期**: 2025-12-28
> **状态**: ✅ 迁移完成 (Phase 1-7 全部完成)
> **下一步**: 删除 TypeScript agents（见 [remove-typescript-agents.md](./remove-typescript-agents.md)）
...
- [7. 迁移计划](#7-迁移计划)
- [8. 验证清单](#8-验证清单)
- [9. 删除 TypeScript Agents 计划](#9-删除-typescript-agents-计划)
...
| **代码保留** | `apps/agents/` (TS) 计划删除（见 Section 9），`apps/agents-py/` 为唯一后端 |
...
┌─────────────────────────────────────────────────────────┐
│  apps/agents/ (TypeScript) - ⚠️ 计划删除                  │
│  └── 详见 Section 9 删除计划                              │
│      迁移完成后将仅保留 Git 历史作为参考                    │
└─────────────────────────────────────────────────────────┘
```
...
## 9. 删除 TypeScript Agents 计划

> **Codex Review 日期**: 2025-12-29
> **完整计划详见**: [remove-typescript-agents.md](./remove-typescript-agents.md)

### 9.1 结论

Python 版本 (`apps/agents-py`) 已 **100% 功能完整**，可以安全删除 TypeScript 版本。
...
**API URL 配置问题**：evals 应直连 Python LangGraph Server (54367)，而非 Next.js 代理 (3000/api)

**解决方案**: 创建 evals 专用 client

```typescript
// packages/evals/src/utils.ts (新文件)
import { Client } from "@langchain/langgraph-sdk";
...
1. 文档清理不完整（`docs/workflow/*.md` 仍引用 `apps/agents`）
2. 缺少 yarn.lock 更新步骤
...
| 文件 | 操作 | 优先级 |
|------|------|--------|
| `packages/evals/package.json` | 移除 `@opencanvas/agents` 依赖 | 高 |
| `packages/evals/src/utils.ts` | **新建** - evals 专用 client | 高 |
| `packages/evals/src/highlights.ts` | **重写** - SDK 完整图调用 | 高 |
| `packages/evals/src/agent.int.test.ts` | **重写** - SDK 完整图调用 | 高 |
| `.github/workflows/ci.yml` | 改用 Python 后端检查 | 高 |
| `yarn.lock` | 更新依赖 | 高 |
...
| 风险 | 级别 | 缓解措施 |
|------|------|---------|
| evals 节点级 API 不兼容 | 高 | 重写为完整图调用 + 断言输出 |
